...
async function getVideo(authority, slug) {
    ...
    const { response, destroy } = request({
        url,
        method: 'GET',
        headers: getHeadersForVideo(author)
    });
    
    const result = response.status === 200 ? response.body : response.status;
    destroy();
